...
def normalize_vertex_list(verts):
    ...
    for v in verts:
        for i in range(0,3):
            ...
            if v[i] <v_min[i]:
                v_min[i] = v[i]
    print('Model min values', v_min)
    print('Model max values', v_max)
    # Generate statistical mean
    mean = [a+b for a,b in zip(v_max,v_min)]
    mean = [a/2.0 for a in mean]
    print('Model center of gravity', mean)
    # translate vertices
    for v in verts:
        for i in range(0,3):
            v[i] = (v[i]-mean[i])/(v_max[i]-v_min[i])
        new_verts.append(v)
...
def normalize_vertex_array(verts):
    ...
    new_verts=normalize_vertex_list(new_verts)
    return new_verts
...
def load(file_name,normalize=True):
    """ Load Wavefront OBJ
    ...
    if normalize:
        verts = normalize_vertex_list(verts)
